...
$szBaseURL = sprintf("%s?keyword=%s", $_SERVER['PHP_SELF'],
$szKeyWord);

$szPageParam = sprintf("keyword=%s", $szKeyWord);
...
$szCmdLine = sprintf('/net/per900a/raid0/plsang/usr.local/bin/python /net/per610a/export/das11f/plsang/codes/yfcc100m/yfcc100m_search.py --key %s', $szKeyWord);
...
{   
    ...
    preg_match($pattern, $arConceptVideos[$ii], $matches);
    
	$szVideoID = strval($matches['id']);
    $szPartName = sprintf('yfcc100m_dataset-%d', $matches['part']);
